...
public class UserService {
	 ...
	 * 
	 * @param userId
	 * @return Map<String, String> 아이디 중복확인의 결과코드, 메세지를 리턴한다.
	 */
	 ...
	 * 
	 * @param userVO
	 * @return boolean @ 회원가입 결과를 리턴한다.회원가입결과가 1 이상일 경우 true, 아닐경우 false
	 */
	...
	public void addValidAttribute(Map<String, String> validatorResult, RedirectAttributes redirectAttributes) {
		for (String key : validatorResult.keySet()) {
			...
			redirectAttributes.addFlashAttribute(key, validatorResult.get(key)); // Object 전달할때 사용(일반 문자열도 가능한다. 큰 특징은
																					// URL에 노출이 안된다는점)
		}
	}
